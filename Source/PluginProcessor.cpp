...
{
    for (auto p : papus)
        p->prepareToPlay (sampleRate); //per ogni elemento di papus, che è costituito da oggetti di tipo PAPUEngine viene eseguita la prepare to play(metodo di PAPUEngine)
}
...
void PAPUAudioProcessor::processBlock (juce::AudioSampleBuffer& buffer, juce::MidiBuffer& midi)
{
    ...
    // il metodo getUserValue di Parameter si trova implementato alla riga 66-69 di gin_parameter.cpp, sostanziamente restituisce il valore del parametro che si ricerca. La variabile value di un parametro è una delle due variabili di stato di gin::Parameter
    if (voices == 1)
    {
        papus[0]->processBlock (buffer, midi);//Se il numero di voci è uguale a 1, viene eseguito il metodo processBlock di PAPUEngine del primo elemento di del vettore papus
    }
    else
    {
        int done = 0;
